...
let auter = document.querySelector('#auteur');
let header = document.querySelector('header').className='ma_class';
// let nav = $('a').css('color','white');
// $('.container-fluid').css('backgroundColor','aliceblue');
$('#navBody').css('backgroundColor','darkseagreen');
$('.nav-link').css('color','black');



// Des varialbes utiles

let dernier = 0
let nomnreAleatoire =0
...
nouveau.addEventListener('click', ()=>{
    do{
     nomnreAleatoire=genererNombreEntier(citations.length)
    } while(nomnreAleatoire==dernier)
    citation.textContent=citations[nomnreAleatoire][0]
    auter.textContent= citations[nomnreAleatoire][1]
    dernier          = nomnreAleatoire
 })